...
layout(matrix(c(1,1,2,2,0,3,3,0), ncol=4,nrow=2, byrow=TRUE), width=c(1,3,3,1),
  ...
  # Plot von n_max
  plot(w, vec_n_max, type="l", main="$n_\\text{max}(w)$", ylab="")

  # Plot von i_N
  plot(w, vec_i_N, type="l", main="$i_N(w)$", ylab="")

  # Plot von t
  # plot(w, vec_t, type="l", main="$t(w)$", ylab="")

  # Plot von MIPS
  plot(w, vec_MIPS_total, type="l", main="$\\text{MIPS}(w)$",
       ylim=c(0,1.5*max(vec_MIPS_total)), ylab="")
  ...
  points(w, vec_MIPS_part4, type="l", lty="dotted")
  points(w, vec_MIPS_part1, type="l", lty="dotted")
  points(w, vec_MIPS_part2, type="l", lty="dotted")
  ...
  legend(x="topright", legend=c("produktbezogene Inputs",  
         ...
         col="black",
         bty="n", inset=0.05, cex=0.8, lty=c(0,0,0,0,1), lwd=c(0,0,0,0,1),
         pch=c(22,22,22,22,NA),
         pt.bg=c(grey(0.80),grey(0.85),grey(0.90),grey(0.95), "black"),
         pt.cex=2)
