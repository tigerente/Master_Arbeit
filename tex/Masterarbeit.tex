...
\todo[color=prioH]{Sternchen von Equations entfernen, damit alle eine Nummer
bekommen}
\chapter{Einleitung}
\section{Thema}
% Problem:
...
Material-Inputs und Definition der Service-Einheit schließlich durch Division
der beiden Größen \parencite[S. 548]{liedtke_resource_2014}:
\begin{equation}
    \MIPS = \frac{\text{Material-Inputs}}{\text{Servicemenge}} =
    \frac{I}{S}
    \label{eq:MIPS_Einleitung}
\end{equation}
In dieser Gleichung werden die also die für ein Produkt aufgewendeten
...
Personen, dem von diesen nachgefragten Service und den für die Befriedigung
dieser Nachfrage eingesetzten Produkten: so kann ein Produktnutzungssystem
...
Beispiel könnten zu einem Produktnutzungssystem für dieselbe Servicenachfrage
die Kunden eines Waschsalons zusammen mit den in diesem Waschsalon aufgestellten
Waschmaschinen gerechnet werden.
...
nicht nur die für den Übergang von herkömmlichen zu gemeinschaftlichen
Nutzungsformen relevanten Faktoren sondern auch Hypothesen über die Richtung
...
herkömmlichen Nutzung mehr Transporte stattfinden. Die Bestimmung der Faktoren
und Hypothesen erweist sich bei der \enquote{Nutzungsdauerverlängerung} 
% und bei dem \enquote{zusätzlichen Ressourcenverzehr für
% Nutzungsdauerverlängerung} 
als etwas komplizierter.  Laut \textcite[S. 57]{scholl_marketing_2009} bedeutet
...
werden als in herkömmlichen Nutzungssystemen. In Tabelle
\ref{tab:faktoren_hypothesen} sind noch einmal
alle der in dieser Arbeit von \textcite{scholl_marketing_2009} betrachteten
Umweltauswirkungen zusammen mit den von uns identifizierten Faktoren und
...
\begin{savenotes}
    \begin{table}[t]
        ...
        \bottomrule
    \end{tabular}
    \caption{Aufgeführt sind die in dieser Arbeit von
        \textcite{scholl_marketing_2009} betrachteten Umweltauswirkungen
    zusammen mit den von uns identifizierten Faktoren und Hypothesen}
    \label{tab:faktoren_hypothesen}
\end{table}
\end{savenotes}

Das Ziel der Modellbildung ist es nun, die Wirkung der genannten Faktoren auf
den Umweltverbrauch eines Produktnutzungssystems zu berücksichtigen. Hierfür
...
Wirkung der beiden Faktoren Nutzungshäufigkeit und Transporte abbildet. Unter
Verwendung der von \textcite{scholl_marketing_2009} aufgestellten Hypothesen über die
Veränderung der Faktoren beim Übergang zu gemeinschaftlichen Nutzungsformen,
sind wir im Sinne einer deduktiven Vorgehensweise in der Lage, 

\todo[inline]{
    - Zur Beantwortung unserer Fragestellung wählen wir ein deduktives Vorgehen:
    Schließen von den genannten Prämissen auf die Umweltauswirkungen. Dafür: \\
    (a) Modell entwickeln für den Umweltverbrauch von Produktnutzungssystemen in
    Abhängigkeit der genannten Faktoren. Konkret: Bestimmungsgleichung für die
    MIPS. Spezifizierung von I und S für ein Produktnutzungssystem. \\ (b)
    Modell anwenden im Sinne der Prämissen, d.h. Untersuchung, was bei
    Veränderung der betrachteten Faktoren mit der MIPS passiert \\ - Leitmotiv
    für die Modellierung: Wäsche waschen. Überleitung zur Fallstudie
}
