...
%%
			...
			%
			\begin{equation*}
				a = \frac{S_D}{N \cdot K} \eqnref{eq:N(h)} \frac{S_D}{h \cdot t \cdot P \cdot K} \eqnref{eq:P(p)} \frac{S_D}{h \cdot T \cdot p \cdot K} \qquad p \in \mathbb{N}
			\end{equation*}
			
			Für ein Nutzungssystem mit gegebener Service-Nachfrage $S_D$, gegebener Nutzungshäufigkeit $h$ und einem festen Betrachtungszeitraum $T$ ist die Produktauslastung $a$ also antiproportional zur parallelen Produktanzahl $p$. Je weniger Produkte parallel im Einsatz sind, desto stärker müssen diese ausgelastet werden um die Nachfrage zu bedienen. Da $p$ ganzzahlig ist, kann die Auslastung $a$ ebenfalls nur diskrete Werte annehmen.
			
			Ebenfalls analog zum Teilmodell \enquote{\nameref{sec:Nutzungsintensivierung}} ergibt sich durch die Beschränkung von $p$ durch $p_\text{min}$ gemäß Gleichung \ref{eq:p_min} eine Maximalauslastung $a_\text{max}$:
			%
			\begin{equation*}
				a = \frac{S_D}{h \cdot T \cdot p \cdot K} \leq \frac{S_D}{h \cdot T \cdot p_\text{min} \cdot K} =: a_\text{max}
			\end{equation*}
...
%
			\begin{align*}
				\text{MIPS}_a &= \frac{I}{S} = \frac{N \cdot i_N(a) + P(a) \cdot i_P + I_{\text{fix}}^a}{S} \\[10pt]
...
%      auch in der Übersicht wiederspiegeln}
			...
			\begin{mdframed}[frametitle={Erhöhung der Produktauslastung}, frametitlerule=true]
				Teilmodell:
				\begin{align}
					\text{MIPS}_a &= \frac{i_N(a)}{a \cdot K} + \frac{P(a) \cdot i_P}{S_D} + \frac{I_{\text{fix}}^a}{S_D} \\[10pt]
				...
				\end{equation}
				\begin{equation}
					a \leq \frac{S_D}{h \cdot T \cdot p_\text{min} \cdot K} =: a_\text{max} \qquad p_\text{min} \in \mathbb{N}
				\end{equation}
