...
In diesem Abschnitt werden ausgewählte Faktoren für den Umweltverbrauch eines Produktnutzungssystems, die im Zusammenhang mit gemeinschaftlicher Produktnutzung relevant sind, isoliert betrachtet. Wir beginnen mit dem Faktor \enquote{Nutzungshäufigkeit}.
     ...
     \end{mdframed}\label{Kriterium:Nutzungshäufigkeit}
      ...
      \subsection{Wartung und Reparatur}\label{sec:Wartung_und_Reparatur}\marginpar{\vspace{-4em}\scriptsize Ende
    ...
    \begin{table}
      ...
      wurde bereits beschrieben, dass Reparaturen zum einen zu einer Verlängerung der
      Lebensdauer führen, mit ihnen jedoch zum anderen eine Erhöhung der
      Material-Inputs einhergeht. Im Abschnitt \nameref{sec:RB} wird für die
      Abbildung dieser beiden Effekte ein Modell entwickelt. Hierfür werden
      zunächst die beiden Parameter Nutzungsdauer und Inputs je Produkt
      (inklusive Reparaturinputs je Produkt) als die beschreibenden Parameter
      für den Faktor Reparatur und die damit einhergehenden Effekte
      herausgearbeitet. Dem schließt sich die Beschreibung einer
      Modellerweiterung an, welche eine funktionelle Abhängigkeit zwischen den
      beiden Größen konstruiert. Das entwickelte Faktor-spezifische Modell wird
      im Anschluss im Abschnitt \nameref{sec:RA} zunächst ohne und dann mit
      Berücksichtigung der Modellerweiterung analysiert. Die Analyse folgt dabei
      der Frage, inwiefern unter der Prämisse von mehr Reparaturen, die
      gemeinschaftliche Produktnutzung einen geringeren Umweltverbrauch als die
      herkömmliche Produktnutzung aufweist.

      \paragraph{Modellbeschreibung}\label{sec:RB}
      Die Abfolge von Defekten und darauf reagierenden Reparaturen bezeichnen
      wir als den Reparaturverlauf eines Produkts. Dieser Verlauf
      ...
      konzeptionell ein wenig von den bisher entwickelten Faktor-spezifischen Modellen
      unterscheidet, hebt sich daher die Notwendigkeit der Unterscheidung
      zwischen einer nutzungsbedingten und einer zeitlich bedingten Entsorgung,
      wie sie im Abschnitt \nameref{sec:Produktnutzungsdauer}
      (\ref{sec:Produktnutzungsdauer}) eingeführt wurde, auf. Wir benutzen in
      diesem Modell zum Faktor Reparatur schlicht den Parameter Nutzungsdauer
      $t$, den wir zur Verdeutlichung der geänderten Sichtweise auch als
      \enquote{erreichte Nutzungsdauer} bezeichnen werden. Der erste zu
      beschreibende Effekt -- eine Verlängerung der Nutzungsdauer -- wird sich
      in der Analyse dann einfach als $t + \Delta t$ widerspiegeln. 
      ...
      beschreibende Effekt dar, die Erhöhung der Material-Inputs. Dieser Effekt
      schlägt sich in den Inputs für Reparaturen $I_R$ nieder, die sich aus der
      Anzahl der im Nutzungssystem während des Betrachtungszeitraums $T$
      ...
      bezieht, ist sie genau wie die Produktanzahl $P$
      als effektive Größe zu verstehen. Im Gegensatz dazu, bezeichnen wir die
      durchschnittliche Anzahl an Reparaturen, die je Produkt über dessen
      gesamte Nutzungsdauer $t$ stattfinden mit $r$. Multipliziert man diese
      Größe mit der effektiven Produktanzahl, so erhält man gerade die Anzahl
      der Reparaturen $R$:
      \begin{equation}
          R=P\cdot r
      \end{equation}
      Die Material-Inputs für Reparaturen $I_R$, können nun, das sie sich auch
      auf die effektive Produktanzahl beziehen, mit den produktbezogenen Inputs
      zu einer Größe zusammengefasst werden: 
      \begin{equation}
          I_{P,R} \defeq I_P + I_R = P \cdot i_P + R \cdot i_R = P \cdot i_P + P \cdot r \cdot
          i_R = P \cdot (i_P + r \cdot i_R) = P \cdot i_{P,R} = \frac{N}{h \cdot
          t} \cdot i_{P,R}
          \label{eq:Reparatur_Sum_I_P_I_R}
      \end{equation}
      In dieser Umformung wurden auch die Inputs je Produkt $i_P$ mit den Inputs
      für Reparaturen an einem Produkt $i_R$ zu einer Größe zusammengefasst. Die
      zusätzlichen Indizes in den Ausdrücken $I_{P,R}$ bzw. $i_{P,R}$ bedeuten
      einfach nur, dass wir zu den produktbezogenen Inputs $I_P$ bzw. Inputs je
      Produkt $i_P$, die bisher die Herstellung, Bereitstellung und Entsorgung
      von Produkten umfassten, nun auch die Reparaturen hinzuzählen. Sprachlich
      werden wir der Einfachheit halber jedoch weiterhin die Begriffe
      \enquote{produktbezogene Material-Inputs} und \enquote{Material-Inputs je
      Produkt} verwenden. Der Effekt einer Erhöhung der Material-Inputs durch
      zusätzliche Reparaturen wird sich in der Analyse nun gerade dadurch
      ausdrücken, dass die Inputs je Produkt um einen Wert $\Delta i_{P,R}$
      vergrößert werden. 

      Es ist zu beachten, dass die produktbezogenen Inputs $I_{P,R}$ nun sowohl
      von der erreichten Nutzungsdauer $t$ als auch von den Inputs je Produkt
      $i_{P,R}$ abhängen.  Dies ist der zufälligen Natur der Reparaturen
      geschuldet, der zufolge kein funktioneller Zusammenhang zwischen den
      beiden Größen angegeben werden kann.  So kann eine bestimmte Nutzungsdauer
      einerseits durch verschiedenartige Reparaturen erreicht wurden sein
      (beispielsweise kann ein Fahrradschlauch geflickt oder ganz ausgetauscht
      werden). Andererseits können im Nutzungsverlauf identischer Produkte ganz
      unterschiedliche Reparaturen notwendig gewesen sein (so kann z.B. eins von
      zwei identischen Fahrrädern in einen Unfall geraten, und somit viel
      umfangreichere Reparaturen erfordern, als das Fahrrad, welches unfallfrei
      geblieben ist).  Das heißt, dass derselben Nutzungsdauer verschieden hohe
      Material-Inputs je Produkt entsprechen können.
      ...
      % Zusammenhang konstruieren: Jedes Produkt besteht aus einer bestimmten
      % Menge an Produktteilen. Bei einer Waschmaschine könnte dies
      % beispielsweise die Wäschetrommel, der Abwasserschlauch, die
      % Waschmittelkammer, usw. umfassen. Wir annehmen, dass einerseits jedes dieser
      % Produktteile eine spezifische Lebensdauer hat, also nach einer bestimmten
      % Zeit defekt ist und dass andererseits jedes dieser Teile ohne
      % Beeinflussung der anderen Teile ausgetauscht werden kann und dafür eine
      % Produktteil-spezifische Menge an Material-Inputs notwendig ist. Weiterhin
      % nehmen wir an, dass das neu eingesetzte Produktteil wieder über dieselbe
      % Lebensdauer wie das ersetzte Produktteil verfügt. Unter diesen Annahmen
      % entspräche dann jede erreichte Nutzungsdauer einem daraus ableitbarem
      % Umfang an Material-Inputs für Reparaturen. 
      %
      % Diese Vorstellung des Reparaturverlaufs eines Produkts ließe sich
      % prinzipiell auch formal in einem Modell abbilden und analysieren. Sie
      % führt aber zu ungefähr denselben Ergebnissen, wie wenn eine weitere, diese
      % Sicht des Reparaturverlaufs noch einmal vereinfachende Annahme getroffen
      % wird: Unter der Annahme, dass es genau ein Produktteil gibt, dass eine im
      % Vergleich zu allen anderen Produktteilen sehr kleine spezifische
      % Lebensdauer hat, kann die Betrachtung des Reparaturverlaufs auf
      % dieses eine Produktteil reduziert werden. Bei einer Waschmaschine könnte
      % das beispielsweise der Wasserschlauch sein, der regelmäßig durch
      % Kalkablagerungen spröde wird und ausgetauscht werden muss. Der Rest der
      % Waschmaschine könnte im Vergleich dazu als verhältnismäßig langlebig
      % angenommen werden.
      %
      % Die spezifische Lebensdauer dieses kurzlebigsten Produktteils entspricht
      % dann der technischen Lebensdauer $\t{tech}$ des Produkts selbst, denn
      % diese war ja gerade als die Dauer bis zur Funktionsuntüchtigkeit des
      % Produkts definiert. Wenn das neu eingesetzte Produktteil wie angenommen
      % wieder eine Lebensdauer von $\t{tech}$ hat, dann ergibt sich bei einer
      % erreichten Nutzungsdauer die Anzahl je Produkt durchgeführter Reparaturen
      % $r$ wie folgt:
      % \begin{equation}
      %     r = \left\lfloor\frac{t}{\t{tech}}\right\rfloor
      % \end{equation}
      % Wenn 
