...
\label{eq:Bedingung_Reparatur}
    ...
    %   \end{equation*}
      ...
      Nutzungsformen darstellen. Transporte können dabei zum einen sehr
      unterschiedlich organisiert sein und zum anderen werden an sie
      produktspezifisch verschiedene Anforderungen gestellt. Um dem gerecht zu
      ...
      Wir bezeichnen die transportbedingten Material-Inputs mit
      $I_\Theta$. Sie ergeben sich aus der Anzahl der
      Transporte $\Theta$ und den Material-Inputs je Transport $i_\Theta$:
      \begin{equation}
          I_\Theta = \Theta \cdot i_\Theta
          \label{eq:Transporte_Inputs_Allgemein}
      \end{equation}
      Die Anzahl der Transporte modellieren wir als abhängig von der vom System
      ...
      hin und zurück zum Waschsalon stattfindet. Natürlich muss hierfür
      definiert sein, was genau einen Transport umfasst.

      Die genaue Beschreibung eines Transports ist auch für die damit
      einhergehenden Material-Inputs wichtig. Diese ergeben sich aus dem
      Transportweg $d$ und den Material-Inputs je Transportweg $i_d$, welche vom
      verwendeten Transportmittel abhängig sind. Würde nun ein Transport
      anstelle der Zusammenfassung von Hin- und Rückweg nur einen der beiden
      Wege umfassen, so würde $\theta$ doppelt so groß, der Transportweg dagegen
      aber nur halb so lang sein, sodass natürlich in der einen wie in der
      anderen Betrachtungsweise die selben Material-Inputs für Transporte
      anfallen. 
      
      Für die korrekte Bilanzierung der transportbedingten
      Material-Inputs muss außerdem berücksichtigt werden, dass der Zweck eines
      Transports häufig nicht auf die alleinige Nutzung des betrachteten
      Produkts zurückzuführen ist. So wird beispielsweise die private Autofahrt
      zum Waschsalon noch mit einem Einkauf im daneben liegenden Supermarkt
      verbunden. Ein anderes Beispiel ist der postalische
      Versand eines Produkts von seinem Lager- zu seinem Nutzungsort: hier
      werden -- z.B. bei der Fahrt eines Postautos -- verschiedenste Produkte
      verschickt, die nichts mit dem betrachteten Produktnutzungssystem zu tun
      haben. Wir tragen dem Rechnung durch einen Korrekturfaktor $\alpha$,
      der angibt, welcher Anteil der transportbedingten Material-Inputs für das
      Nutzungssystem angerechnet wird, es gilt also $\alpha \in [0,1]$. Eine
      Möglichkeiten diesen Korrekturfaktor zu operationalisieren, ist die
      Betrachtung der Anzahl der unterschiedlichen Zwecke, die je Transportwege
      erfüllt werden. Als Korrekturfaktor könnte dann der Kehrwert dieser Anzahl
      genommen werden.  Wird beispielsweise die Fahrt zum Waschsalon mit der
      Fahrt zur Arbeit und der Fahrt zum Einkaufen kombiniert, so würde $\alpha$
      gemäß dieser Operationalisierung ein Drittel betragen.
      Die transportbedingten Material-Inputs lassen sich nun folgendermaßen
      berechnen:
      \begin{equation}
          i_\Theta = d \cdot i_d \cdot \alpha
          \label{eq:Transporte_Inputs_je_Transport}
      \end{equation}
      Wir fassen
      jetzt die Gleichungen \eqref{eq:Transporte_Inputs_Allgemein},
      \eqref{eq:Transporte_Anzahl} und \eqref{eq:Transporte_Inputs_je_Transport}
      zusammen und erhalten die allgemeine Gleichung für die Transportinputs:
      \begin{equation}
         I_\Theta = N \cdot \theta \cdot d \cdot i_d \cdot \alpha
          \label{eq:Transporte_Inputs}
      \end{equation}
      Wir betrachten diese Gleichung nun unter dem Gesichtspunkt der eingangs
      vorgestellten Prototypen:
      \subparagraph{Dimension Produkt: Vergleich von Waschmaschine und
      Bohrmaschine} 
      Um zentral aufgestellte Waschmaschinen nutzen zu können, muss die Wäsche
      zunächst dorthin transportiert werden. Ein Charakteristikum dieser
      Produktnutzung ist daher, dass die gesamte im Nutzungssystem zu
      transportierende Menge, welche wir als $M$ bezeichnen, nur von der
      Servicenachfrage $S_D$ abhängt. 
      Diese Menge ist insbesondere unabhängig von der Transporthäufigkeit, denn
      egal wie häufig oder selten ein Transport stattfindet, es muss immer die
      gleiche Menge an Wäsche transportiert werden. Wir können daher formal
      schreiben:
      \begin{equation*}
          M = m_S \cdot S_D 
      \end{equation*}
      ...
      Da die Gesamttransportmenge $M$ konstant in der Transporthäufigkeit ist,
      muss die Transportmenge je Transport, welche wir mit $M_\Theta$
      bezeichnen, mit der Häufigkeit der Transporte
      variieren. Formal ergibt sich die Transportmenge je Transport 
      aus der Gesamttransportmenge $M$ geteilt durch die Anzahl der
      Transporte $\Theta$:
      \begin{equation}
          M_\Theta = \frac{M}{\Theta} = \frac{m_S \cdot S_D}{\theta \cdot N}
          \label{eq:Transport_Menge_je_Transport}
      \end{equation}
      Eine andere Formulierung des eingangs formulierten Charakteristikums
      lautet also, dass die Transportmenge je Transport indirekt proportional
      zur Transporthäufigkeit ist.
      
      Genau andersherum verhält es sich bei der Bohrmaschinennutzung. Um eine
      ...
      damit beliebig viele Löcher gebohrt werden. Ein Transport kann also zur
      Realisierung verschieden vieler Serviceeinheiten führen, weshalb genau
      gegenteilig zur Waschmaschinennutzung, bei der Bohrmaschinennutzung die
      Gesamttransportmenge $M$ mit den Anzahl Transporten variiert. Genauer
      ergibt sich die Gesamttransportmenge als Produkt aus Tranpormenge je
      Transport $M_\Theta$ und Anzahl der Transporte $\Theta$:
      \begin{equation*}
          M = M_\Theta \cdot \Theta = M_\Theta \cdot \theta \cdot N
      \end{equation*}
      Das Charakteristikum der Bohrmaschinennutzung ist also, dass die
      Gesamttransportmenge proportional zur Transporthäufigkeit ist. Eine
      analoge Charakterisierung erhält man wiederum bei Betrachtung der 
      Einzeltransportmenge:
      \begin{equation*}
          M_\Theta = konst.
      \end{equation*}
      Die Einzeltransportmenge ist konstant in der Transporthäufigkeit,
      da je Transport genau eine Bohrmaschine transportiert werden muss.

      \subparagraph{Dimension Transport: Vergleich von privatem Transport mit
      Lieferservice}
      Privater Transport ist dadurch charakterisiert, dass das
      ...
      relevanten Veränderungen der Inputs je Transportweg $i_d$ führen. Gemäß
      der Operationalisierung von $\alpha$ bezüglich der Zwecke je Transportweg,
      ergibt sich außerdem, dass $\alpha$ unabhängig von der
      Einzeltransportmenge ist. Insgesamt ergibt sich somit, dass die Inputs je
      Transport $i_\Theta$ konstant in der Transportmenge je Tranport sind:
      \begin{equation*}
          i_\Theta = d \cdot i_d \cdot \alpha = konst.
      \end{equation*}

      Im Gegensatz zum privaten Transport, kann bei einem Lieferservice davon
      ausgegangen werden, dass die durchschnittliche Auslastung der
      Transportmittel konstant ist: zwar kann bei einem Transportweg mehr oder
      weniger von der betrachteten Transportmenge transportiert werden, diese
      verdrängt jedoch andere Produkte, die auch transportiert werden sollen. 


      \begin{mdframed}[frametitle={Transporte}, frametitlerule=true]
          Teilmodell:
          \begin{equation}
              \MIPS[d] = \frac{\theta \cdot d \cdot i_d}{a \cdot K} +
              \frac{\I{fix}^d}{S_D}
              \label{eq:MIPSTransporte}
          \end{equation}
          Alternatives Teilmodell:
          \begin{equation}
              \MIPS[d] = d \cdot i_{d, M} \cdot m_S +
              \frac{\I{fix}^d}{S_D}
