...
\section{Methodisches Vorgehen}
        ...
        % Theorie: gut: allgemeine Aussagen, schlecht: Aussagen nur nach dem
        % Schema "`unter der und der Voraussetzung.."' 
        % => Fallstudie, adaptierte Fragestellungen 
        % Methodisches Vorgehen: Szenarien
        Wissenschaftliche Forschung bewegt sich seit jeher im Spannungsfeld
        zwischen Theorie und Empirie. Da wo die Theorie bei Wenn-Dann-Aussagen
        stehen bleibt, setzt die Empirie mit Feststellungen über die Natur der
        Dinge an. Und da wo die Empirie über Einzelfälle nicht hinauskommt,
        erhebt die Theorie den Anspruch der Verallgemeinerung. 

        Unsere gewählte Fragestellung (vgl. Kapitel \ref{sec:Fragestellung})
        verlangt eher nach einer theoretischen Herangehensweise, denn nach einer
        empirischen. Um jedoch die Anwendbarkeit der auf diesem Wege gewonnenen
        Erkenntnisse zu überprüfen und Beschränkungen der Theorieentwicklung
        herauszuarbeiten, möchten wir einige ausgewählte Modelle in konkreten
        Fallbeispielen einsetzen. Das Leitthema soll hier weiterhin die
        gemeinschaftliche Waschmaschinennutzung sein. Die übergeordnete
        Fragestellung die wir an die Fallbeispiele stellen lautet:

        \begin{quote} 
            Kann der Umweltverbrauch (in den Fallbeispielen) durch die
            gemeinschaftliche Nutzung von Waschmaschinen gegenüber ihrer
            individuellen Nutzung reduziert werden?
        \end{quote}

        Zur Beantwortung dieser Frage werden wir auf die im Theorieteil
        entwickelten Kriterien zurückgreifen.  Das Interesse bei der Fallstudie
        liegt jedoch nicht alleinig darin, konkrete Aussagen über realistische
        Szenarien treffen zu können. Die Fallbeispiele erlauben es darüber
        hinaus auch, einen Einblick in die im Allgemeinen nicht interessante, im
        Speziellen dagegen relevante Größenordnung der betrachteten Effekte zu
        gewinnen und hierbei ein -- im Hinblick auf die verschiedenen
        MIPS-Dimensionen -- differenzierteres Bild zu zeichnen.

        Wir beschreiben nun das konkrete Vorgehen zur Beantwortung der
        gestellten Fragestellung und definieren insbesondere die gewählten
        Fallbeispiele. 

        Die Fallstudie umfasst zwei Analyseteile. Im ersten Analyseteil
        werden die Modelle Nutzungsintensivierung und Transporte sowie deren
        allgemeine \todo{wie wird die erste Kopplung bezeichnet} Kopplung
        angewendet. Hierzu werden die MIPS von je zwei Szenarien miteinander
        verglichen, von denen eines die Rolle der \enquote{individuellen
        Nutzung} und eines die Rolle der \enquote{gemeinschaftlichen Nutzung}
        einnimmt\footnote{\enquote{Individuelle Nutzung} bzw.
        \enquote{gemeinschaftliche Nutzung} sind dabei lediglich als
        Rollenbezeichnungen zu verstehen. Ersteres steht für das
        Ausgangsszenario, letzteres für das Zielszenario. Das heißt
        insbesondere, dass im Szenario \enquote{individuelle Nutzung} nicht je
        Person genau eine Waschmaschine genutzt wird.}. Wir definieren die
        folgenden Szenarien, die sich an unserer eigenen Wohnsituation im
        Studierendenwohnheim \enquote{Luhrmannhof} orientieren:
        ...
        \begin{description}
            ...
            \item[Szenario 3: Waschsalon] Auch in diesem Szenario leben die 50
                ...
                Kilometer. 
                \todo{Szenario genauer spezifizieren}
                % ca. ein Drittel der Studierenden besitzt ein Auto
        \end{description}
        ...
        den Szenarien identisch sind. Außerdem nehmen wir an in diesem und
        ...
        Aus den drei Szenarien ergeben sich die in der Tabelle
        \ref{tab:Fallbeispiele} angegebenen drei Kombinationsmöglichkeiten, die
        für die Anwendung je eines der Modelle Verwendung finden.
        % \begin{description}
        %     \item[Fallbeispiel Nutzungsintensivierung:] Vergleich von Szenario
        %         1: Kleine Wohneinheiten mit Szenario 2: Luhrmannhof. Die
        %         Szenarien unterscheiden sich in der Produktanzahl.
        %     \item[Fallbeispiel Transporte:] Vergleich von Szenario 2:
        %         Luhrmannhof mit Szenario 3: Waschsalon. Die 
        %         Szenarien unterscheiden sich in der mittleren Transportdistanz.
        %     \item[Fallbeispiel allgemeine Kopplung:] Vergleich von Szenario 1:
        %         Kleine Wohneinheiten mit Szenario 3: Waschsalon. Die Szenarien
        ...
        \begin{table}
            \footnotesize
            \centering
            \begin{tabular}[h]{p{3.2cm}p{3cm}p{3cm}p{3cm}}
                \toprule
                Modell      & individuell & gemeinschaftlich &
                variierter Parameter \\
                \midrule
                Nutzungsintensivierung & Kl. Wohneinheiten & Luhrmannhof & Produktanzahl \\
                Transporte & Luhrmannhof & Waschsalon & Transportdistanz \\
                allgemeine Kopplung & Kl. Wohneinheiten & Waschsalon & beide\\
                \bottomrule
            \end{tabular}
            \caption{Unter \enquote{individuell} und \enquote{gemeinschaftlich}
            sind die Szenarien angegeben, die die entsprechende Rolle einnehmen.
            Die Szenarien unterscheiden sich in einem oder zwei Parametern und
            eignen sich dann für die Anwendung des zugehörigen Modells.}
            \label{tab:Fallbeispiele}
        \end{table}

        Im zweiten Analyseteil geht es um die Anwendung des Modells für die
        spezielle Kopplung von Nutzungsintensivierung und Transporten. Dieses
        Modell besteht auch aus einem individuellen und einem gemeinschaftlichen
        Szenario, die in diesem Modell jedoch parallel auftauchen. Wir
        orientieren uns in ihrer Defintion an den Szenarien 1 und 3 aus dem
        ersten Analyseteil:
        \begin{description}
            \item[Szenario individuelle Nutzung] Im Stadtgebiet Osnabrücks leben
                xxx Studierende zu je fünf Personen in einer Wohngemeinschaft,
                die sich je eine Waschmaschine teilen.
            \item[Szenario gemeinschaftliche Nutzung] Das Studentenwerk eröffnet
                einen Waschsalon im Zentrum Osnabrücks. In einem Einzugsgebiet
                mit variabler Größe verzichten die Studierenden auf die eigene
                Waschmaschine und nutzen stattdessen den studentischen
                Waschsalon. In dem Waschsalon werden in Abhängigkeit von der
                Anzahl der Nutzer\_innen genau so viele Waschmaschinen
                aufgestellt, dass diese mit einer vorgegeben Nutzungsintensität
                betrieben werden.
        \end{description}

        Eine Übersicht über die in den Fallstudien verwendeten Daten und deren
