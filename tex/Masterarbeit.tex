...
\section{Methodisches Vorgehen}
        ...
        An die theoretische Modellbildung schließt sich eine Fallstudie zu dem
        Bereich der gemeinschaftlichen Nutzung von Waschmaschinen an, welche
        einen Teil der gewonnenen Ergebnisse exemplarisch auf konkrete
        Fallbeispiele anwendet. In den Fallbeispielen sollen die Effekte
        Nutzungsintensivierung und Transporte zunächst einzeln und dann in ihrer
        Kopplung untersucht werden. Auch hier ist die der Arbeit zugrunde
        liegende Fragestellung leitend für die Untersuchung, erscheint jedoch in
        der folgenden adaptierten Variante:
        \begin{quote}
            Kann der Umweltverbrauch in den Fallbeispielen durch die
            gemeinschaftliche Nutzung gegenüber der individuellen Nutzung
            reduziert werden?
        \end{quote}
        % immer ein Szenario als "`individuelle Nutzung"' und eins als die Rolle
        % der "`gemeinschaftlichen Nutzung"'

        
        In einem ersten Analyseteil betrachten wir
        dabei die MIPS-Änderung zwischen je zwei der folgenden Szenarien:
        ...
        \end{description}
        Szenario 2 hat dabei die Szenarien motiviert, da dieses unserer eigenen
        Wohnsituation in dem Wohnheim \enquote{Luhrmannhof} entspricht. Für die
        Betrachtung der Effekte werden die folgenden Fallbeispiele betrachtet:
        ...
        Ein zweiter Analyseteil besteht darin, 


        % in allen Szenarien gleich: 
        % * 50 Studierende leben in zehn 5er-WGs
        % * in allen Szenarien werden dieselben Waschmaschinen benutzt
        % * die Auslastung sowie die Inputs für Reparaturen und Wartungen und
        % auch die fixfix Inputs sind identisch

        % Beschreibung der Szenarien:
        %    - im wesentlichen zwei Analyseteile: 
        %       1) drei Szenarien, deren Unterschiede untersucht werden 
        %       2) studentischer Waschsalon in der Stadt Osnabrück
        % => Fragestellungen für die Fallstudien
        % Weitere Zwecke der Fallstudie:
        % * Demonstrierung der Anwendbarkeit:
        %   -wie können die Ergebnisse für reale Fragestellungen eingesetzt
        % werden
        %   - welche Fragestellungen können mit den Modellen beantwortet werden
        % * Illustrierung / Greifbarmachen der abstrakten Theorie
        % * Einblick in die Größenordnungen der Effekte
        % * Blick auf die unterschiedlichen Dimensionen der MIPS
        % * Validierung: Können die theoretisch entwickelten Modelle sinnvoll
