...
In diesem Abschnitt werden ausgewählte Faktoren für den Umweltverbrauch eines Produktnutzungssystems, die im Zusammenhang mit gemeinschaftlicher Produktnutzung relevant sind, isoliert betrachtet. Wir beginnen mit dem Faktor \enquote{Nutzungshäufigkeit}.
  ...
  $S_D$ vereinheitlichen}
      ...
      Aus der Wartungshäufigkeit ergibt sich durch Multiplikation mit den
      ...
      sofern dies zu einer Verlängerung der Nutzungsdauer führt, auf die
      ...
      aufgestellten Grundgleichungen $n = h \cdot
      t$ \eqref{eq:Grundgleichung_n} und $P = \frac{N}{n}$ \eqref{eq:Grundgleichung_P}:
      \begin{align}
          I_P(w) &= i_P \cdot P(w) \eqnref{eq:Grundgleichung_P} i_P \cdot
          \frac{N}{n(w)} \eqnref{eq:Grundgleichung_n} i_P \cdot \frac{N}{h \cdot
          t(w)} \\[10pt]
                 &= 
        \left\{ 
      ...
      % Gleichung \eqref{eq:Grundgleichung_P}): 
      %
      % \begin{equation}
      ...
      Material-Inputs mit zunehmender Wartungshäufigkeit abnimmt: \todo{checken
      ob die Beschränkung Verwendung findet}
      ...
      \begin{align}
          ...
          &= N \cdot (i_N(w) +  w \cdot i_W)
            + \frac{N}{h\cdot t(w)} \cdot i_P + \I{fix}^w\label{eq:I(w)2}
      \end{align}
...
\begin{description}
    ...
    \item[Datenblatt der Waschmaschinen:] Die Anleitung der bei uns im
        Wohnheim verwendeten Waschmaschinen beinhaltet eine Übersicht über den
    ...
    \item[Literatur:] Weiterhin fanden Daten aus den Studien von 
        \textcite[S. 57]{hirschl_produkte_2000} und von 
        \textcite[S. 12]{behrendt_nachhaltig_2000} Verwendung.\\
        \textit{Parameter: Einzelnutzungsvorrat, Maximalnutzungsdauer}
...
werden, sondern, dass alle Parameter um $1\%$ erhöht oder erniedrigt werden, so dass in beiden
Anwendungsbeispielen eine \emph{größt mögliche} MIPS-Einsparung erzielt wird.
Das Plus-Zeichen steht also nicht für die Richtung der Parametervariation sondern für das
Resultat der Parametervariation hinsichtlich der MIPS. Ebenso bedeutet eine
Variation um $-1\%$, dass alle Parameter um $1\%$ erhöht oder erniedrigt werden,
sodass sich in den Anwendungsbeispielen eine \emph{kleinst mögliche} MIPS-Einsparung (bzw. dann auch
eine MIPS-Erhöhung) ergibt. Entsprechendes gilt für die Paramtervariationen um
$\pm 5\%$ und $\pm 10\%$.
...
Für die Anwendung der Ergebnisse aus dem Modell zur Nutzungshäufigkeit wird in diesem
...
\SzenarioI\ ist das Szenario mit herkömmlicher Nutzung und das \SzenarioII\
ist das Szenario mit gemeinschaftlicher Nutzung. Ersters wird im Text alternativ
auch als Ausgangsszenario und letzteres auch als Zielszenario bezeichnet.
...
vergleichen. Mit $h^*$ wurde dabei 
die Nutzungshäufigkeit bezeichnet, bei
der die Maximalnutzungsdauer der technischen Lebensdauer entspricht. Als
Ungleichung ausgedrückt (vgl. \todo{Referenz}), liegt eine Senkung des
Umweltverbrauchs durch gemeinschaftliche Nutzung genau dann vor, wenn gilt:
\begin{equation}
    h^\text{herk.} < h^*  = \frac{\n{max}}{\t{max}}
\end{equation}
...
\begin{table}[t]
    ...
    \caption{Übersicht über die Kriterienüberprüfung im Anwendungsbeispiel zur
        ...
        für die Parametervariationen. Die parallele
        ...
        Referenzwerte ($= 0\%$ Parametervariation)
    gegeben.}
...
ist. Würden in diesem Szenario anstatt der zwei zehn Waschmaschinen eingesetzt,
wäre -- zumindest bei reiner Betrachtung des Effekts der Nutzungsintensivierung
-- die erzielte Ressourceneinsparung in allen Fällen und Unsicherheitsbereichen
dieselbe.
% \begin{table}[ht] \centering \input{Tabellen/NI_Kriterien.tex} \caption{test}
% \label{tab:NI-Kriterien} \end{table}
...
gehabt das Szenario mit herkömmlicher Nutzung, welches alternativ als
Ausgangsszenario bezeichnet wird.
...
Bezeichnung \enquote{Zielszenario} verwendet.
