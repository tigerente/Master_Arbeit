...
\begin{minipage}[t]{0.45\textwidth}
    \onehalfspacing
    \emph{Zweiter Analyseteil}:
    \begin{itemize}
        \item  Radius R
        \item  Bevölkerungsdichte und Dichte der potentiellen Nutzer\_innen
        \item  Nutzungsintensität im Waschsalon
        \item  Nachfrage pro Person
    \end{itemize}
\end{minipage}
    \todo{die richtigen Daten des Kopplungsmodells eintragen}
...
Eine Senkung des Umweltverbrauchs durch gemeinschaftliche Nutzung liegt gemäß
\ldots\todo{Referenz} dann vor, wenn gilt:
\begin{equation*}
    h^\text{herk.} < h^*  = \frac{\n{max}}{\t{max}}
\end{equation*}
...
verschieden. Sie sind in der Tabelle \ref{tab:NI-Kriterien} aufgeführt.
...
\begin{table}[t]
    ...
    \caption{Übersicht über die Kriterienüberprüfung im Anwendungsbeispiel zur
        ...
        Produktanzahl im Szenario \textsc{Kleine Wohneinheiten} beträgt 17. Ist dieser
...
herkömmlichen Nutzung. Für das Anwendungsbeispiel ist durch das Szenario
\textsc{Luhrmannhof} auch die Produktanzahl für die gemeinschaftliche Nutzung definiert.
...
im Szenario \textsc{Luhrmannhof} die Reduktion an Produkten deutlich über das Ziel
...
MIPS-Reduktion gegenüber dem Szenario \textsc{Kleine Wohneinheiten} in
...
festgehalten werden, dass zwar von einer Ressourceneinsparung im Szenario
\textsc{Luhrmannhof} im Vergleich zum Szenario \textsc{Kleine Wohneinheiten}
...
\subsubsection{Kriterienüberprüfung}
% - d*  berechnen für die verschiedenen Cases und mit d_Ind
%   vergleichen
		\begin{figure}[ht]
...
\subsubsection{Größenordnung der MIPS-Einsparung}
% beide Graphiken bringen (also delta Mips (d) und Säulendiagramm
% für erstere Graphik:
% - ansonsten gleiches Schema wie oben
% - zusätzlich noch auf die Unsicherheitsbereiche in beiden Dimensionen eingehen
