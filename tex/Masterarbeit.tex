...
konst.$ gilt. Beim Übergang zu $\T{eff}$ müssen wir berücksichtigen, dass
wir
...
werden:
\begin{equation}
  \I{fix, eff} = T \cdot \itext{fix} = konst.
\end{equation}
...
unterscheiden müssen. 
	...
	%
				...
				\begin{equation*}
					P (h) = \frac{N}{n(h)} = \left\{\begin{array}{cl}  \frac{N}{h \cdot t_{\text{max}}}, & \mbox{falls } h < h^* =  \frac{n_{\text{max}}}{t_{\text{max}}} \\[5pt] \frac{N}{n_{\text{max}}}, & \mbox{sonst} \end{array}\right.
				\end{equation*}
				...
				\begin{equation}
				\label{P(a)}
					P (a) = \frac{N(a)}{n(a)} = \left\{\begin{array}{cl} \frac{S}{a \cdot K \cdot h \cdot t_{\text{max}}}, & \mbox{falls } n_\text{max}(a) > h \cdot t_\text{max} \\[5pt] \frac{S}{a \cdot K \cdot n_{\text{max}}(a)}, & \mbox{sonst} \end{array}\right.
				\end{equation}
	...
	%
				\begin{align*}
					...
					& \eqnref{P(a)} \frac{i_N(a)}{a \cdot K} + \frac{I_{\text{fix}}^a}{S_D} + \left\{\begin{array}{cl} \frac{i_P}{a \cdot K \cdot h \cdot t_{\text{max}}}, & \mbox{falls } n_\text{max}(a) > h \cdot t_\text{max} \\[5pt] \frac{i_P}{a \cdot K \cdot n_{\text{max}}(a)}, & \mbox{sonst} \end{array}\right.
				\end{align*}	
	...
	%      auch in der Übersicht wiederspiegeln}
				...
				\begin{mdframed}[frametitle={Erhöhung der Produktauslastung}, frametitlerule=true]
					...
					\begin{align}
						...
						&= \frac{i_N(a)}{a \cdot K} + \frac{I_{\text{fix}}^a}{S_D} + \left\{\begin{array}{cl} \frac{i_P}{a \cdot K \cdot h \cdot t_{\text{max}}}, & \mbox{falls } n_\text{max}(a) > h \cdot t_\text{max} \\[5pt] \frac{i_P}{a \cdot K \cdot n_{\text{max}}(a)}, & \mbox{sonst} \end{array}\right.
					\end{align}
					Nebenbedingungen:
					\begin{equation}
			    ...
			    In diesem Abschnitt wird eine Bedingung für einen positiven Umwelteffekt einer geringfügigen Erhöhung der Produktauslastung hergeleitet. Dabei wird davon ausgegangen, dass die Anzahl $p$ der parallel eingesetzten Produkte gleich bleibt. Ein denkbares Szenario im Bereich von Waschmaschinen ist, dass zwei Personen jeweils eine eigene Waschmaschine besitzen, jedoch dazu übergehen, gelegentlich eine der beiden Maschinen gemeinsam zu nutzen, um je Waschgang eine höhere Auslastung zu erzielen.

			    Der Umwelteffekt kann durch die Ableitung der MIPS nach der Produktauslastung bestimmt werden:
   				%
   				\begin{equation}
	   				\label{eq:d_MIPS_d_a}
	   				\frac{\partial \MIPS[a]}{\partial a} = \frac{i_N'(a)\cdot a - i_N(a)}{a^2 \cdot K} + \left\{\begin{array}{cl} 0, & \mbox{falls } a > a^* := \frac{S_D \cdot t_\text{max}}{n_\text{max} \cdot K \cdot p \cdot T} \\[5pt] \frac{i_P}{a^2 \cdot K \cdot n_{\text{max}}}, & \mbox{sonst} \end{array}\right.
   				\end{equation}
   				%
	   			Es lässt sich nun untersuchen, unter welchen Bedingungen eine geringfügige Erhöhung der Produktauslastung zu einer Verringerung der MIPS führt. Dazu werden die beiden Fälle $a > a^*$ und $a \leq a^*$ separat behandelt.

	   			\underline{Fall 1:} $a > a^*$

	   			Dies ist der Fall, in dem der Nutzungsvorrat $n_\text{max}$ aufgrund einer zu geringen Nutzungshäufigkeit $h$ nicht innerhalb der Maximalnutzungsdauer $t_\text{max}$ aufgebraucht wird. Hier gilt nach Gleichung \ref{eq:d_MIPS_d_a} der folgende Zusammenhang:
	   			%
   				\begin{equation*}
			    ...
			    Im zweiten Fall wird der Nutzungsvorrat innerhalb der Maximalnutzungsdauer aufgebraucht. Hier liefert Gleichung \ref{eq:d_MIPS_d_a} die folgende Bedingung:
   	   			...
   	   			\end{equation*}
   	   			%
			    Da der Ausdruck $\frac{i_P}{n_\text{max} \cdot i_N(a)}$ echt positiv ist, stellt die Elastizitäts-Bedingung für $i_N(a)$ aus Fall 1 auch in Fall 2 eine hinreichende Bedingung für die Verringerung der MIPS dar. Darüber kommt es hier auch dann zu einer Verringerung der MIPS, wenn zwar die Elastizitäts-Bedingung verletzt wird, die Inputs pro Produkt $i_P$ jedoch hinreichend groß sind. Dies liegt an der Einsparung an sequentiell eingesetzten Produkten in Fall 2.

			    \subparagraph{Einsparung parallel eingesetzter Produkte}
			    Die vorherige Analyse beruhte auf der Annahme einer unveränderlichen Anzahl parallel eingesetzter Produkte. Jedoch eröffnet eine hinreichend große Erhöhung der Produktauslastung die Möglichkeit, die Zahl der parallel eingesetzten Produkte zu reduzieren und dadurch eine weitere Ressourceneinsparung zu erzielen. Dieser Effekt wird in der folgenden Analyse untersucht.
			   ...
			   Es handelt sich bei $p(a)$ also um eine Treppenfunktion. Im Rahmen dieser Analyse betrachten wir deren Sprungstellen, also solche Werte für $a$, bei denen $p_\text{min}(a)$ ganzzahlig ist. Für die Bereiche dazwischen, in denen $p(a)$ konstant ist, lässt sich die obige Analyse (\enquote{\nameref{sec:Auslastung_Analyse1}}) anwenden, sodass alle dort erarbeiteten Ergebnisse auch hier gültig sind.
