...
\end{titlepage}
\tableofcontents
\todo{Gender: Sternchen verwenden}
\chapter{Einleitung}
\section{Thema}
% Beispiel
\begin{itemize}
    \item Beispiel(e) für gemeinschaftliche Produktnutzung
    \item Anschluss zu folgendem Absatz
\end{itemize}
...
verdeutlichen, dass bei der herkömmlichen Nutzung die maximale Nutzungsdauer und
Auslastung von Produkten häufig nicht erreicht werden. 

% Lösung:
Formen gemeinschaftlicher Nutzung, um die es in unserer Arbeit geht, können
durch Nutzungsintensivierung und Erhöhung der Auslastung die daraus
resultierenden Effizienzpotentiale heben und dadurch zu einer Reduktion der
Umweltbelastung führen. Zahlreiche dieser Nutzungsformen, die sich über alle
Produktgruppen erstrecken, sind bereits seit geraumer Zeit etabliert. Zum einen
finden sich hier alle Formen des Gebrauchtwarenmarkts von Second-Hand-Läden für
Kleidung über Geschäfte des An- und Verkaufs bis hin zu Flohmärkten. Zum anderen
existiert eine breite Palette an Miet- und Verleihsystemen, wie zum Beispiel
Bibliotheken, Videotheken oder Autovermietungen. Neben diesen kommerziellen
Angeboten gibt es auch vielfältige Formen der privaten gemeinschaftlichen
Nutzung, wie beispielsweise die gemeinsame Nutzung von Waschmaschinen,
Rasenmähern oder Kühlschränken in Wohnanlagen und Wohngemeinschaften, oder die
...
Smartphones ganz einfach das nächste Free Floating Car zu finden. \todo{konkrete
Webadressen/Projektnamen für die Beispiele hinzufügen}
...
Wohlstand führen können. Aus Unternehmensperspektive stellen sich diese
...
\todo[inline]{
    - Unterschiedliche Begriffe für ähnliche Phänomene\\ 
    - Quellen für die unterschiedlichen Begriffe angeben\\
    - Unterschiede und Gemeinsamkeiten besser herausarbeiten\\ 
    - am Ende: "`Wir verwenden den Begriff \enquote{gemeinschaftliche
    Produktnutzung}, da \dots"' \\ 
    - Definition des Begriffs:
        - zu einem Zeitpunkt: definierter Nutzer*innen-Kreis
        - mehrere Personen benutzen gemeinsam ein oder mehrere Produkte, das
        kann gleichzeitig oder abwechselnd sein
        - privat oder professionell organisiert
        - sich wiederholende / über einen längeren Zeitraum stattfindende
        Benutzung von Produkten 
    , dabei insbesondere: Ausklammern von Second Hand
    - ab da, nur noch mit diesem Begriff arbeiten (nicht mehr Sharing Economy
    verwenden
}

Im Vergleich zur Euphorie, die von der Sharing Economy ausgeht, zeigt sich aus
wissenschaftlicher Perspektive hinsichtlich der Umweltwirkungen
gemeinschaftlicher Nutzungssysteme ein differenzierteres Bild: Zum einen werden
die positiven Primäreffekte der Nutzungsintensivierung und Erhöhung der
Auslastung um
weitere sekundäre Ressourceneinsparmöglichkeiten ergänzt. So kann es
beispielsweise bei der gemeinschaftlichen Nutzung einer Waschmaschine sinnvoll
sein, eine halbgewerbliche Maschine mit entsprechend längerer Lebensdauer und
höherer Effizienz zu verwenden. Zum anderen werden die durch gemeinschaftliche
Nutzung ermöglichten Ressourceneinsparungen jedoch auch durch negative
Umweltwirkungen begleitet: Beispielsweise müssen für die Koordination der
gemeinschaftlichen Nutzung und den Transport der genutzten Produkte zwischen den
Nutzer*innen zusätzliche Ressourcen aufgewendet werden. Und auch positive
sekundäre Effekte können mit negativen Nebenwirkungen einhergehen, wie
beispielsweise für die Produktion einer halbgewerblichen Waschmaschine im
Vergleich zu einer handelsüblichen mehr Ressourcen benötigt werden.
\todo{primär/sekundär rausstreichen; - am Ende Quelle einfügen}
...
\todo[inline]{- das vorige zusammenfassen (vielfältige Effekte);\\
              - auf Thema der Masterarbeit eingehen: wann positiver Effekt? \\
              - Kriterien aufstellen\\
              - zu diesem Zweck: Modellentwicklung}
...
Umwelteffekte. Er gliedert diese in solche, die direkt durch die Nutzungsform
auftreten und solche, die indirekt aus veränderten Nachfragemustern resultieren (vgl. Tabelle \ref{tab:Mechanismen}).
...
\textcite{erdmann_quantifizierung_2011}  genau das anhand des online-basierten
Gebrauchtwarenhandels.

\todo[inline]{
    - in folgendem Absatz: berücksichtigen, dass wir Second Hand Nutzungen
    explizit ausgeschlossen haben
    - trotzdem darüber schreiben
}
Auch in der ökonomischen Literatur lassen sich Autor*innen finden, die sich mit
gemeinschaftlichen Nutzungsformen -- unter Verwendung einer deutlich
formalisierteren Methodik -- auseinandersetzen. \textcite{yokoo_economic_2010}
...
der Gebrauchtwarenhandel mit Büchern betrachtet.

\todo[inline]{
    Die folgenden beiden Absätze inhaltlich beibehalten, aber irgendwie besser
    einbauen\\
    - eigentlich gibts schon relativ viel
    - aber der Aspekt den wir behandeln, ist in der zitierten Literatur nur einer unter
    vielen\\
    - es fehlt: Theoretische Betrachtung / allgemeine Prinzipien
    - außerdem: Ressourcenseite (Verweis auf Methodenkapitel)
}
In den angegebenen Publikationen werden neben den Umweltwirkungen je nach
Studie zahlreiche weitere Aspekte behandelt -- wie beispielsweise
Handlungsempfehlungen, Strategien zur Förderung, mögliche Hemmnisse,
...
Nutzungsformen} sowohl auf empirischer als auch auf theoretisch-konzeptioneller
Ebene bereits einige Forschung betrieben wurde, auf die wir mit unserer Arbeit
aufbauen können. In Hinblick auf einige Aspekte sehen wir jedoch weiteren
Forschungsbedarf. Zum einen beschränkt sich die Quantifizierung der
Umwelteffekte in den meisten angegebenen Quellen auf spezielle Produkte und
Nutzungsformen. Eine Ausnahme bilden die formalen Modelle der ökonomischen
Fachliteratur, die allerdings wiederum nur spezielle Umwelteffekte abbilden.  Es werden in der Regel
Einzelfallberechnungen durchgeführt, ohne den Einfluss der (implizit)
verwendeten Parameter zu untersuchen. Was die Zielgrößen angeht, so konzentriert
sich ein Großteil der Literatur auf Emissionen, während die Ressourcen-Seite
eher vernachlässigt wird. 
		...
		\end{table}
        \todo{Tabelle wieder an das Original anpassen}
\section{Forschungsziel und Fragestellung}\label{sec:Fragestellung}
\todo[inline]{
    - Anknüpfen an den Teil davor\\
    - Dann: Hauptziel = Kriterien \dots\\
    - nicht produktspezifisch/nutzungsformspezifisch sondern allgemein\\
    - Danach: Fragestellung\\
    - Als letztes: sekundäres Ziel: Betrachtung des eingangs angesprochenen
    Fallbeispiels\\
}
Das Hauptziel unserer Arbeit ist die Entwicklung von Kriterien für die Senkung des Umweltverbrauchs durch gemeinschaftliche Produktnutzung. 

\todo{Fragestellung in Box packen}
\begin{itemize}
  \item Unter welchen Umständen kann der Umweltverbrauch eines Produktes durch
      gemeinschaftliche Nutzungsformen gesenkt werden?
\end{itemize}
\section{Methodisches Vorgehen}
\subsection{Operationalisierung}
\todo[inline]{
    - für die Betrachtung der Umwelteffekte benötigen wir einen Vergleichspunkt,
    das ist bei uns das herkömmliche\\
    - herkömmliche Produktnutzungen können auch gemeinschaftlich
    organisiert sein\\
    - die betrachteten "`gemeinschaftlichen"'
    Nutzungsformen sind jedoch noch "`gemeinschaftlicher"', im Sinne
    einer höheren Personenanzahl pro Produkt oder je Nutzung\\
    - Umweltverbrauch: MIPS-Konzept (\cite{schmidt-bleek_maia:_1998}) --> hier
    MIPS-Gleichung einführen\\
}
\subsection{Modellierung}
\todo[inline]{
    - Konzeption: \\
        a) Produktnutzungssystem (Personen + Produkte + Organisation der
        Nutzung)\\
        b) entspricht der systemwissenschaftlichen Perspektive: System, Grenzen,
        Elemente, Relationen \\
...
\label{eq:Bedingung_Reparatur}
    ...
    %   \end{equation*}
      ...
      weniger Produkte eingesetzt werden als Nutzer\_innen am System teilnehmen,
      zeichnen sich diese im Besonderen durch eine Entkopplung der
      Produktstandorte von den Wohnorten der Nutzer\_innen aus. Mit Transporten
      ...
      \end{figure}
		...
		Nachdem im vorigen Kapitel ausgewählte Umwelteffekte durch gemeinschaftliche Produktnutzung isoliert untersucht wurden, soll nun analysiert werden, wie mehrere dieser Effekte zusammenwirken. Aus pragmatischen Gründen musste eine Auswahl von zwei Einzeleffekten getroffen werden, deren Kopplung im Rahmen dieser Master-Arbeit betrachtet werden kann.
		 ...
		 Hinter diesem Ansatz steht der folgende Grundgedanke: Eine Nutzungsintensivierung durch gemeinschaftliche Produktnutzung wird dadurch ermöglicht, dass mehrere Personen ein Produkt gemeinsam nutzen. Dafür ist es in vielen Anwendungen erforderlich, dass entweder die Personen zum Produkt oder das Produkt zu den Personen transportiert werden. Beispielsweise müssen die Nutzer\_innen eines Waschsalons sich selbst und die zu waschende Wäsche von ihrem Wohnort zum Waschsalon transportieren, wohingegen bei einem versandbasierten Werkzeugverleih das geliehene Werkzeug zu den nutzenden Personen transportiert werden muss.
