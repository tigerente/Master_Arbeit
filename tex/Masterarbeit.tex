...
\begin{mdframed}[frametitle={Grundmodell}, frametitlerule=true]
	...
	\begin{minipage}[t]{0.49\textwidth}
        ...
        \frac{n_\text{max}}{h}\} \label{eq:Produktnutzungsdauer}\\
...
\end{mdframed}		
	...
	%
				\begin{align}
					...
					\eqnref{eq:h(p)} \frac{i_P}{t_\text{max} \cdot A} \cdot \frac{A \cdot T}{S_D} \cdot \frac{\frac{1}{p_1} - \frac{1}{p_2}}{\frac{1}{p_1 \cdot p_2}}  \\[10pt]
...
%				\end{equation}
	...
	%      auch in der Übersicht wiederspiegeln}
      ...
      untersucht. Dazu werden im Folgenden zunächst die Faktoren definiert sowie
      deren Relevanz für gemeinschaftliche Nutzungssysteme begründet. Daran
      anschließend werden die Gemeinsamkeiten und Unterschiede zwischen den
      beiden Faktoren herausgestellt um schließlich zu der Feststellung zu
      kommen, dass es trotz Übereinstimmung in den wesentlichen umweltrelevanten
      Effekten sinnvoll ist, die beiden Faktoren getrennt voneinander zu
      betrachten. Daher folgen diesem einleitenden Teil zwei Unterabschnitte, in
      denen getrennt voneinander der Einfluss von Wartung und von Reparaturen
      auf den Umweltverbrauch von Produktnutzungssystemen beschrieben und
      analysiert wird.

      Wartungen und Reparaturen verbindet der gemeinsame Zweck, die Lebensdauer
      von Produkten zu verlängern. Darüber hinaus bezeichnen diese Faktoren
      jedoch unterschiedliche Tätigkeiten, die nun definiert werden.

      Unter dem Begriff "`Wartung"' verstehen wir eine Pflegetätigkeit an
      ...
      Fahrradkette) oder der Reinigung von durch die Nutzung
      verschmutzten Produktteilen (z.B. Flusensieb 
      einer Waschmaschine) vor. 
      ...
      Wie aus den Beispielen bereits ersichtlich wurde, sind Wartungen und
      Reparaturen nicht auf gemeinschaftliche Nutzungssysteme beschränkt,
      sondern sind auch in herkömmlichen Nutzungssystemen üblich. Laut der im
      Abschnitt \nameref{sec:Modellierung} (\ref{sec:Modellierung})
      herausgearbeiteten Hypothese über die Faktoren Wartung und Reparatur,
      werden in gemeinschaftlichen Nutzungssystemen jedoch \emph{mehr} Wartungs-
      und Reparaturtätigkeiten als in herkömmlichen Nutzungssystemen
      durchgeführt.  Zu den diese Hypothese stützenden Argumenten zählt
      ...
      wird (und somit ansich notwendiger wird), dass in Gemeinschaften mehr Wissen
      über Reparatur- und Wartungsmöglichkeiten vorhanden ist und dass
      ...
      Ressourcenbilanz zusätzlich verbessern. Hinter dem Umwelteffekt
      \enquote{Wartung und Reparatur} verbergen sich also in Wahrheit drei
      verschiedene Effekte, die in unterschiedlichen Kombinationen auftreten.
    ...
    Umwelteffekten. Sie sind Gegenstand der folgenden Modellbildung.}
      ...
      \todo[inline]{einleitende Worte mit Bezug auf die folgenden Abschnitte}
  ...
  $S_D$ vereinheitlichen}
      ...
      als Operationalisierung der Umweltauswirkung durch Wartungstätigkeiten, 
      formal beschreiben.\todo{letzten Satz nochmal lesen}

      Aus der Wartungshäufigkeit ergibt sich als Produkt der Wartungshäufigkeit
      und den Nutzungseinheiten die Anzahl Wartungen $W$.  Der erste Effekt, die
      Erhöhung der Material-Inputs lässt sich nun direkt formulieren:
      \begin{equation}
      ...
      Abschnitt zur Nutzungsintensivierung
      \ref{sec:Nutzungsintensivierung}\todo{Referenz checken, wenn Modell zur
      Nutzungshäufigkeit fertig ist} beschriebene Zusammenhang zwischen der
      Produktnutzungs-Menge $N$ und der Produktanzahl $P$:
      \begin{equation}
        N = h \cdot t(w) \cdot P
      \end{equation}
