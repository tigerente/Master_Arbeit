...
Im dritten Fall, der sich von den anderen beiden Fällen durch die Bedingung $h^\text{herk} \geq h^*$ unterscheidet, ist die kritische Distanz $0$. Das heißt, eine Verringerung des Umweltverbrauchs ist für keine zusätzliche Distanz gegeben, wenn der Nutzungsvorrat eines einzelnen Produkts bereits vor der Erhöhung der Nutzungshäufigkeit ausgeschöpft wird. Dies liegt daran, dass die Erhöhung der Nutzungshäufigkeit in diesem Fall keine Einsparung von Produkten zur Folge hat, wie im Modell zur Nutzungshäufigkeit bereits herausgearbeitet wurde.

Die Ausdrücke im ersten und zweiten Fall sind bis auf den Vorfaktor $-\frac{1}{\mathbb{T}}$ bereits aus dem Modell zur Nutzungshäufigkeit bekannt (vgl. Gleichungen \eqref{eq:Delta_MIPS_h_1} bzw. \eqref{eq:Delta_MIPS_h_3}, beachte unterschiedliche Reihenfolge der Fälle). Dort wurde herausgearbeitet, dass sie negativ sind, also ist die kritische Distanz $\hat{d}$ hier durch den Vorfaktor positiv. Das heißt im Fall $h^\text{herk} < h^*$, also wenn der Nutzungsvorrat eines einzelnen Produkt vor der Erhöhung der Nutzungshäufigkeit \emph{nicht} ausgeschöpft wird, gibt es eine echt positive kritische Transportdistanz, bis zu der insgesamt eine Verringerung des Umweltverbrauchs erreicht wird.

Wir können die ersten beiden Fälle unter der Bedingung einer gegebenen Nutzungshäufigkeit $h^\text{herk}$ bei der herkömmlichen Produktnutzung mit $h^\text{herk} < h^*$ zu einer Funktion für die kritische Distanz zusammenfassen, die nur noch von $h^\text{gem}$ abhängt:
\begin{equation}
   \hat{d}(h^\text{gem}) = \left\{ \begin{array}{ll}
      \frac{i_P}{\mathbb{T} \cdot t_\text{max} \cdot A} \cdot \left( \frac{1}{h^\text{herk}} - \frac{1}{h^\text{gem}} \right) & \mbox{falls } h^\text{gem} < h^* \\[10pt]
      \frac{i_P}{\mathbb{T} \cdot A} \cdot \left( \frac{1}{h^\text{herk} \cdot t_\text{max}} - \frac{1}{n_\text{max}} \right) & \mbox{falls } h^\text{gem} \geq h^* 
   \end{array} \right.
\end{equation}

Der Ausdruck in Fall 1 wächst in $h^\text{gem}$, das heißt, solange der Nutzungsvorrat in der gemeinschaftlichen Produktnutzung noch nicht ausgeschöpft wird, steigt die kritische Distanz mit der Nutzungshäufigkeit. Der Ausdruck in Fall 2 ist konstant, das heißt eine Erhöhung der Nutzungshäufigkeit über den Punkt $h^*$ hinaus hat keinen weiteren Effekt. Insofern lässt sich der Ausdruck in Fall 2 auch als die maximale kritische Distanz für eine gegebene Nutzungshäufigkeit $h^\text{herk}$ in der herkömmlichen Produktnutzung auffassen.

Abbildung \ref{fig:delta_d_h_gem} stellt die Funktion $\hat{d}(h^\text{gem})$ dar. Im weißen Bereich mit dem Plus-Symbol wird eine MIPS-Einsparung erreicht, im grauen Bereich mit dem Minus-Symbol kommt es zu einer Erhöhung der MIPS. Gut erkennbar ist das Erreichen der maximalen kritischen Distanz bei $h^*$.

\begin{figure}[ht]
   \input{Abbildungen/Kopplung_Allgemein.tex}
   \caption{MIPS-Reduktion in Abhängigkeit der Nutzungshäufigkeit $h^\text{gem}$ bei der gemeinschaftlichen Produktnutzung und der zusätzlichen Transportdistanz $\Delta d$. Als Linie eingezeichnet ist die kritische Distanz $\hat{d}$. Die weiße Fläche kennzeichnet den Bereich der MIPS-Einsparung, im grauen Bereich findet eine Erhöhung der MIPS statt. Parameter: $i_P$~=~5.000~kg, $\mathbb{T}$~=~0,5~kg/km, $t_\text{max}$~=~15~Jahre, $n_\text{max}$~=~5.000~NE, $A$~=~1~SE/NE, $h^\text{herk}$~=~200~NE/Jahr (NE = Nutzungseinheiten, SE = Service-Einheiten).}
