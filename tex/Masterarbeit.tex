...
\chapter{Fallstudie}
        % Außerdem nehmen wir an in diesem und
        % auch im nächsten Analyseteil an, dass Material-Inputs sich aus den drei
        % Bestandteilen nutzungsbedingte Inputs, produktbezogene Inputs und
        % transportbedingten Inputs ergeben und darüber hinaus keine weiteren
        % Ressourcenverbräuche anfallen.
Wir kommen nun zur Anwendung der theoretischen Erkenntnisse auf das Beispiel der
gemeinschaftlichen Nutzung von Waschmaschinen. Im ersten Abschnitt stellen wir zunächst
die für die Berechnung verwendeten Daten und deren Datenquellen dar. Im zweiten
Abschnitt führen wir dann die Beispielrechnungen für die Modelle
Nutzungsintensivierung und Transporte sowie deren Kopplung durch. Im dritten
Abschnitt diskutieren wir schließlich die Ergebnisse.
\section{Daten}
...
\begin{minipage}[t]{0.45\textwidth}
    ...
    \begin{itemize}
        \item  nachgefragte Service-Menge $S_D$
        ...
        \item  Produktauslastung $a$
    \end{itemize}
\end{minipage}
% alternativ: eine lange Liste:
% \begin{itemize}
%     \item Inputs:
%         \begin{itemize}
%             \item  Material-Inputs je Produkt $i_P$
...
Die Material-Inputs sind insofern besonders, als dass sie mehrdimensionale
Vektoren darstellen, da sie für die verschiedenen MIPS-Dimensionen
unterschiedliche Werte annehmen.

Neben den Szenario übergreifenden werden werden desweiteren die folgenden
\emph{Szenario spezifischen Daten} für die Beispielrechnungen benötigt:

\begin{minipage}[t]{0.45\textwidth}
    \onehalfspacing
    \emph{Erster Analyseteil}:
    \begin{itemize}
...
Die folgenden Tabellen stellen alle für die Beispielrechnungen verwendeten Daten
und deren Quelle dar:

        % Das Stadtgebiet wird für die Anwendung des Modells als Kreisfläche
        % mit einer homogenen Bevölkerungs- und Studierendenverteilung
        % idealisiert.
...
\input{Tabellen/Fallstudie_Datentabelle.tex}
    ...
    % \end{longtable}
\subsection{Sensitivitätsanalyse / Min-Max-Abschätzung / Parametervariation}
