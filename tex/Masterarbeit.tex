...
\todo[inline]{
    - Konzeption: \\
        ...
        Elemente, Relationen \\
    - für den Umweltverbrauch dieser Produktnutzungssysteme entwickeln wir ein
    Modell: eine Bestimmungsgleichung für die MIPS \\
    - für die Analyse der Senkung des Umweltverbrauchs vergleichen wir die MIPS
    von zwei Systemen (herkömmliches mit gemeinschaftlichem System, die den
    selben Service erbringen)\\
    - Scholl beschreibt, worin sich diese Systeme unterscheiden können\\
    - wir orientieren uns an den von ihm aufgestellten Effekten\\
    - Wichtig: Wir gehen vom Auftreten dieser Effekte aus (z.B. Nutzungshäufigkeit und/oder Auslastung sind höher bei gemeinschaftlicher Nutzung) und untersuchen deren ökologischen Konsequenzen
    - je Effekt ein Modell entwickeln und dieses für sich analysieren\\
    - allgemein treten mehrere Effekte gleichzeitig auf: daher Betrachtung von
    Kopplung\\
    - Auswahl der Effekte + Kopplung\\
    - Erklärung der neuen Bezeichnungen\\
...
\label{eq:Bedingung_Reparatur}
    ...
    %   \end{equation*}
      ...
      \end{figure}
		...
		\section{Effekt-Kopplung: Nutzungsintensivierung und zusätzliche Transporte}
...
\end{figure}
		...
		Anders als bei der isolierten Betrachtung der Nutzungsintensivierung können unter der gleichzeitigen Berücksichtigung zusätzlicher Transporte sowohl Verbesserungen als auch Verschlechterungen der MIPS auftreten. Als hartes Kriterium für eine Senkung des Umweltverbrauchs erweist sich eine hinreichend geringe Nutzungsintensität im Ausgangsniveau, d.h. vor der Nutzungsintensivierung: Nur wenn der Nutzungsvorrat ursprünglich nicht voll ausgeschöpft wird ($h_1 < h^*$), kann es zu einer Reduzierung der MIPS kommen. In diesem Fall muss jedoch die weitere Bedingung gelten, dass die Materialinputs für die zusätzlichen Transporte kleiner sind als die eingesparten produktbezogenen Inputs für ein parallel eingesetztes Produkt ($\alpha \cdot S_D \cdot \Delta d < q \cdot i_P$).
		 ...
		 \subsection{Spezifisches Modell}
