...
\usepackage[textsize=tiny, colorinlistoftodos, disable]{todonotes} % TODO-Notizen. Ausschalten mit der Option [disable]
...
kann, wenn mehr als eine Person das Produkt nutzt. Jedoch wird sie in diesem
Fall mit einer Nutzungsform verglichen, die \emph{stärker} gemeinschaftlich
...
MIPS-Ansatz, der von (\cite{schmidt-bleek_maia:_1998}) ausführlich beschrieben
...
Die Input-Orientierung drückt sich dadurch aus, dass für die Bestimmung der MIPS
...
Mit der Service-Orientierung, wird im MIPS-Konzept der Fokus weg von dem
...
Die Quantifizierung des Umweltverbrauchs erfolgt nach Bestimmung der
Material-Inputs und Definition sowie Bestimmung der Service-Menge schließlich
durch Division der beiden Größen \parencite[S. 548]{liedtke_resource_2014}:
\begin{equation}
    \MIPS = \frac{\text{Material-Inputs}}{\text{Service-Menge}} =
    \frac{I}{S}
...
Die Gleichung \eqref{eq:MIPS_Einleitung} bildet die Grundlage für unsere
Modellentwicklung.

\subsection{Modellierung}
Die mathematische Modellierung stellt unser zentrales Werkzeug für die
Beantwortung der Fragestellung dar. Bevor wir jedoch erläutern, wie wir mit
...
Als Gegenstand der Modellbildung betrachten wir Produktnutzungssysteme. Die
Elemente des Systems sind Personen und Produkte, welche über eine bestimmte
Nutzungsform in Beziehung stehen. Die Systemgrenze orientiert sich somit an den
Personen und den für die Befriedigung ihrer Nachfrage nach einem bestimmtem
Service eingesetzten Produkten: so kann ein Produktnutzungssystem
beispielsweise die Personen eines Haushalts umfassen, welche zur Befriedigung
des Bedürfnisses nach sauberer Wäsche eine Waschmaschine benutzen. Als weiteres
Beispiel könnten zu einem Produktnutzungssystem die Kunden eines Waschsalons
zusammen mit den in diesem Waschsalon aufgestellten Waschmaschinen gezählt
werden.

Jedem derart definierten Produktnutzungssystem kann nun ein Umweltverbrauch
zugeschrieben werden, der von Parametern des Systems abhängt. Einige dieser
Parameter sind davon abhängig, ob dem System eine herkömmliche oder
gemeinschaftliche Nutzungsform zugrunde liegt. Diese Parameter bezeichnen wir
als Faktoren. Aus den von \textcite{scholl_marketing_2009} beschriebenen
\textit{Umweltauswirkungen} (siehe Tabelle \ref{tab:Mechanismen}) lassen sich
sowohl die für den Übergang von herkömmlichen zu gemeinschaftlichen
Nutzungsformen relevanten Faktoren als auch Hypothesen über die Richtung
deren Änderung ableiten.  Wenn \textcite{scholl_marketing_2009} zum Beispiel von
einer \enquote{Maximierung der Geräteauslastung} spricht, so beinhaltet dies
...
Wir beziehen uns in unserer Arbeit auf die folgenden von
...
zugehörige Hypothese zerlegt werden kann, wurde bereits beschrieben. Ähnlich
...
Wie wir im Abschnitt \todo{Referenz} noch im Detail erläutern werden, liegt dem
ersten hier angesprochenen Prozess -- die Annäherung der tatsächlichen
Nutzungsdauer an die Lebensdauer -- der Parameter Nutzungshäufigkeit, also die
Häufigkeit mit der ein Produkt während einer Zeitperiode benutzt wird, zugrunde.
Eine Annäherung der Nutzungsdauer an die Lebensdauer geschieht unter einer
Erhöhung der Nutzungshäufigkeit, die Hypothese im Zusammenhang mit der als
...
Nutzungshäufigkeit erhöht. Dem zweiten Prozess, den
...
\begin{savenotes}
    \begin{table}[t]
        ...
        \begin{tabular}{p{5cm}p{4cm}p{4cm}}
            ...
            Faktor &
            Hypothese zur gemeinschaftlichen Nutzung \\
            \midrule
            Maximierung der\newline Geräteauslastung &
            Produktauslastung\footnote{Im Text wurde zwar von
                \enquote{Geräteauslastung} gesprochen, wir verwenden statt
                dessen jedoch im folgenden den Begriff
                \enquote{Produktauslastung}, da Produkte als etwas allgemeineres
                Konzept, die Elemente der von uns betrachteten Nutzungssysteme
            sind.  }
        & erhöht sich\\
    ...
    \caption{Aufgeführt ist unsere Auswahl der von
        \textcite{scholl_marketing_2009} identifizierten Umweltauswirkungen
        sowie deren Zerlegung in Faktoren und Hypothesen.}
...
\ref{tab:faktoren_hypothesen} zusammenfassend dargestellt. Es ist insofern
zentral für unsere Arbeit, als dass die hier genannten Faktoren die Grundlage
für unsere Modellbildung bilden und die Hypothesen über die Veränderung dieser
Faktoren beim Übergang zu gemeinschaftlichen Nutzungsformen die Analyse der
Faktor-spezifischen Modelle leiten. Dieses Vorgehen wird nun im
letzten Teil dieses Abschnitts erläutert:
...
spezifizieren diese zunächst für jeden Faktor separat, schließen dieser
Einzelbetrachtung aber auch eine Modellkopplung an, welche die kombinierte
Wirkung der beiden Faktoren Nutzungshäufigkeit und Transporte abbildet. 

Für die Beantwortung der Fragestellung, können wir jedoch nicht bei der
Bestimmung des Umweltverbrauchs eines Produktnutzungssystems in Abhängigkeit von
den Faktoren stehen bleiben, sondern müssen noch den Bezug zur
gemeinschaftlichen Produktnutzung herstellen. Hierfür dienen uns die 
Hypothesen über die Faktoränderung bei: nehmen wir deren Gültigkeit als Prämisse
an, und bestimmen die daraus resultierende Änderung des Umweltverbrauchs, so
können wir in einem deduktiven Schritt darauf schließen, unter welchen Umständen
sich der Umweltverbrauch bei gemeinschaftlicher Produktnutzung verringert.

Dieser Logik entsprechend, werden wir die für die Faktoren spezifizierten
Modelle analysieren: wir betrachten eine Änderung der Faktoren entsprechend der
Hypothesen und bestimmen die daraus resultierende Änderung des Umweltverbrauchs
in Abhängigkeit von den sonstigen, das System beschreibenden Parametern. Daraus
ergeben sich Bedingungen an die Paramter, unter denen die resultierende Änderung
des Umweltverbrauchs negativ ist, also eine Senkung des Umweltverbrauchs
vorliegt. Diese Bedingungen sind dann die von uns gesuchten Kriterien für eine
Senkung des Umweltverbrauchs bei gemeinschaftlicher Nutzung.

Um dieser Vorgehensweise etwas von ihrer Abstraktheit zu nehmen und
die Anwendbarkeit der Modelle auf reale Produktnutzungssysteme zu gewährleisten,
orientieren wir uns bei der Modellierung immer wieder an Beispielen.
Insbesondere werden dabei Beispiele zur gemeinschaftlichen Waschmaschinen-Nutzung
eine Rolle spielen, da diesem Thema auch in einer der Theorieentwicklung
anschließenden Fallstudie nachgegangen wird. Das Konzept und Vorgehen der
...
Aussagen über die Vor- bzw. Nachteilhaftigkeit eines Fallbeispiels zu treffen,
...
unserer eigenen Wohnsituation im Studierendenwohnheim \enquote{Luhrmannhof}
ausgehen:
...
beiden Größen Maximalnutzungsdauer und technischer Lebensdauer gegeben. Als
...
\end{mdframed}		
	...
	%
				...
				\end{equation}
	%						
				gegeben. Betrachtet man Gleichung \eqref{eq:N(h)} ($N (h) = h \cdot t \cdot P$) unter diesem Gesichtspunkt, so wird deutlich, dass eine Erhöhung der Nutzungshäufigkeit $h$ bei gleichbleibender Produktnutzungsdauer $t$ zu einer geringeren effektiven Produktanzahl $P$ führt. Hierin besteht der positive Umwelteffekt der Nutzungsintensivierung, da weniger Produkte für die gleiche Service-Menge hergestellt und entsorgt werden müssen.
	...
	%			
				...
				\end{equation}	
	%
				Die erste Bedingung impliziert unter Berücksichtigung von Gleichung \eqref{eq:n(h)}:
				\begin{equation}	
	...
	%
				...
				Für die Nutzungsmenge $n$ eines Produkts ergibt sich hieraus mit Gleichung \eqref{eq:n(h)}:
	%			
				\begin{equation}
					\label{eq:n(h)_cases}
	...
	%			
				...
				Mit Gleichung \eqref{eq:N(h)} ($N = n \cdot P$) lässt sich schließlich die effektive Produktanzahl $P(h)$ bestimmen:
	%			
				\begin{equation}
				\label{eq:P(h)}	
				...
				Für die Service-Menge $S$ wurden mit den Gleichungen \eqref{eq:S(A)} und \eqref{eq:S_D} bereits zwei Bestimmungsgleichungen eingeführt:	$S = N \cdot A$ und $S = S_D$. Unter Berücksichtigung von Gleichung \eqref{eq:N(h)} lässt
	...
	%
				...
				In dieser Analyse des Teilmodells zur Nutzungsintensivierung wird untersucht, wie sich eine Erhöhung der Nutzungshäufigkeit auf die MIPS auswirkt. Da die Nutzungshäufigkeit $h$ gemäß Nebenbedingung \eqref{eq:h(p)} nur in diskreten Schritten variiert werden kann, wird die Änderung der MIPS von einer gegebenen Nutzungshäufigkeit ($h = h_1$) zur nächstgrößeren ($h = h_2$) betrachtet. Diese ist durch die Verringerung der parallelen Produktanzahl von $p = p_1$ auf $p = p_2$ um eins gegeben: $p_2 - p_1 = -1$.
				
				Mit Gleichung \eqref{eq:MIPS(h)} ergibt sich für die Änderung der MIPS:
				\begin{equation}
	...
	%
				...
				Der dritte Fall stellt als Sonderfall den Übergang vom ersten zum dritten Fall dar: Vor der Erhöhung der Nutzungshäufigkeit wird der Nutzungsvorrat eines einzelnen Produkts nicht voll ausgeschöpft, nach der Erhöhung jedoch schon. Für die Veränderung der MIPS gilt nach Gleichung \eqref{eq:MIPS(h)_2}:
	%
				\begin{equation}
...
Beginn Teil von Alexander\newline Müller}
      ...
      Gleichung \eqref{eq:Nutzungsdauer}): 
...
\begin{figure}[ht]
    ...
    \label{fig:Vergleich_Szenarien}
\end{figure}
\chapter{Ergebnisse}\marginpar{\vspace{-6em}\scriptsize Ende Teil von
Alexander\newline Müller
\vspace{1mm} \rule{\marginparwidth}{1pt}
Beginn \newline gemeinsamer Teil}
