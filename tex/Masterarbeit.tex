...
%
			...
			\end{equation}
%
			So stellt beispielsweise eine Waschmaschine, die zwei mal pro Woche genutzt wird, in einer Nutzungsdauer von 10 Jahren $n = 2 \textfrac{Waschgänge}{Woche} \cdot 52 \textfrac{Wochen}{Jahr} \cdot 10 \text{ Jahre} = 1040$ Waschgänge zur Verfügung.
...
%
			...
			\end{equation}
%			
			Hierbei ist zu beachten, dass die Produktanzahl $P$ sowohl gleichzeitig, als auch nacheinander eingesetzte Produkte berücksichtigt. Diese Größe muss nicht ganzzahlig sein, da ein Produkt auch anteilig dem betrachteten Nutzungssystem angerechnet werden kann, wenn es innerhalb des Betrachtungszeitraums nur einen Teil seines Nutzungsvorrats zur Verfügung stellt und der Rest auf eine vorausgegangene oder nachfolgende Nutzung entfällt. Werden beispielsweise in einem Gemeinschaftswaschkeller im Betrachtungszeitraum $T  = 15 \text{ Jahren}$ parallel zwei Maschinen betrieben, die jeweils über eine Nutzungsdauer von $t = 20 \text{ Jahren}$ verfügen, so sind sie jeweils nur zu $\nicefrac{15}{20}$ dem betrachteten Nutzungssystem anzurechnen, sodass sich eine Produktanzahl von $P = 2 \cdot \nicefrac{15}{20} = 1.5$ ergibt. Allgemein gilt:
%
			\begin{equation}
				\label{eq:P(p)}
				P = p \cdot \frac{T}{t}
...
%			
			...
			\noteJ{Abbildung erstellen, einfügen, beschriften und beschreiben!}
