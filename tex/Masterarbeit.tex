...
\section{Modell}
		\subsection{Einzeleffekte}
			...
			Unter dem Begriff \enquote{Produktauslastung} verstehen wir die Intensität, mit der ein Produkt während einer einzelnen Nutzung eingesetzt wird. Im Fall einer Waschmaschine entspricht dies dem Gewicht der Beladung während eines Waschgangs. Bei Büchern könnte analog die Anzahl der Personen, die durch Vorlesen gleichzeitig in den Genuss eines Buches kommen, betrachtet werden. Im Folgenden bleiben wir beim Beispiel der Waschmaschine, da die Relevanz des Vorlese-Szenarios als vernachlässigbar angesehen werden kann.
			
			Formaler betrachtet ist die Produktauslastung $A$ damit der Service, der durch eine einzelne Produktnutzungseinheit erzielt wird -- im Fall der Waschmaschine also das Gewicht der gewaschenen Wäsche pro Waschgang. Nimmt man an, dass ein Produkt über eine gewisse Kapazität $K$ (die maximale Auslastung) verfügt, so kann zur Betrachtung einer relativen Produktauslastung $a$ übergegangen werden: $a = A/K$. Im Fall einer Waschmaschine wäre die Kapazität das maximale Beladungsgewicht und die relative Produktauslastung der Anteil des tatsächlichen am maximalen Beladungsgewicht während eines Waschgangs.
...
%
			...
			\end{equation}
%
			In dieser Gleichung spiegelt sich der offensichtlichste Effekt einer Erhöhung der Produktauslastung wieder, nämlich eine proportionale Verringerung der Produktnutzung bei gleichbleibendem Service. Diesem positiven Effekt treten jedoch mitunter zwei negative Effekte gegenüber: zum einen ein Mehrbedarf an Inputs während der Nutzung und zum anderen eine Reduktion des Nutzungsvorrats.
			
			Ein Mehrbedarf an Inputs während der Nutzung fällt dann an, wenn durch eine erhöhte Produktauslastung mehr Energie oder andere Betriebsmittel eingesetzt werden müssen. So steigt bei einer Waschmaschine beispielsweise der Bedarf an elektrischer Energie und an Waschmittel, wenn die Maschine voller beladen wird. Formal kann dieser Zusammenhang durch eine funktionale Abhängigkeit der Inputs pro Nutzungseinheit $i_N$ von der Produktauslastung $a$ beschrieben werden:
%
			\begin{equation}
				i_N = i_N (a)
			\end{equation}
%
			Wir gehen davon aus, dass eine erhöhte Auslastung mit höheren oder wenigstens gleichbleibenden Inputs pro Nutzungseinheit einhergeht. Das heißt, der Funktionsverlauf von $i_N (a)$ kann als monoton steigend angenommen werden:
%
			\begin{equation}
				\label{eq:monotones i_N}
				\frac{d i_N}{d a} \geq 0
			\end{equation}
			
			Eine Reduktion des Nutzungsvorrats $N_{\text{max}}$ tritt dann auf, wenn eine erhöhte Produktauslastung zu einer verstärkten Abnutzung des Produkts führt. So wird eine Waschmaschine beispielsweise nach einer geringeren Anzahl von Waschgängen aus der Nutzung genommen werden müssen, wenn sie im Durchschnitt voller beladen wurde. Auch dieser Zusammenhang kann durch eine funktionale Abhängigkeit abgebildet werden:
%
			\begin{equation}
				\label{N_max(a)}
...
%
			\begin{equation}
				\frac{d N_{\text{max}}}{d a} \leq 0
			\end{equation}


			Um die Abhängigkeit des MIPS von der Produktauslastung einer Untersuchung zugänglich zu machen, sind zunächst die beiden Komponenten des MIPS, die Materialinputs und die Serviceeinheiten, näher zu betrachten. Die Materialinputs $I$ setzen sich aus nutzungsabhängigen Inputs $I_N$ und nutzungsunabhängigen Inputs $I_{\text{fix}}$ zusammen, wobei unter $I_{\text{fix}}$ hier alle Inputs zur Bereitstellung und Entsorgung des Produkts zusammengefasst werden:
%
			\begin{equation}
				I=N \cdot i_N + I_{\text{fix}}
			\end{equation}			

			Auf der Seite der Serviceeinheiten haben wir mit Gleichung \ref{eq:S(a)} bereits eine Bestimmungsgleichung gegeben: $S=N \cdot a \cdot K$. Diese muss der Nebenbedingung genügen, dass die abgerufenen Nutzungseinheiten $N$ den Nutzungsvorrat $N_{\text{max}}$ nicht übersteigen:
%
			\begin{equation}
			...
			Die Kapazität $K$ wird in diesem Teil-Modell in Bezug auf $a$ als konstant angenommen. Sie ist als Kapazität eines einzelnen Produkts zu verstehen, selbst wenn das betrachtete Produktsystem mehrere Produkte gleichen Typs umfasst. Die abgerufenen Nutzungseinheiten $N$ stellen in diesem Modell eine Zwischengröße dar, die durch die folgenden Umformungen eliminiert werden kann.
			
			Andererseits besteht in einem konkreten Nutzungssystem eine bestimmte Nachfrage nach Serviceeinheiten. Diese wird von der Anzahl der Personen, dem individuellen Nutzungsbedarf und dem Betrachtungszeitraum abhängen, was in diesem Teil-Modell aber nicht weiter von Belang ist. Die Service-Nachfrage $S_D$ ist eine von der Produktauslastung unabhängige Größe und kann insofern hier als Konstante betrachtet werden:
%
			\begin{equation}
				S = S_D
			\end{equation}
...
%
			\begin{equation}
				a \cdot N_{\text{max}}(a) \geq \frac{S_D}{K}.
			\end{equation}
%
			Nun liegen alle Voraussetzungen vor um das MIPS für eine gegebene Auslastung $a$ zu bestimmen:
%
			\begin{align}
				\text{MIPS}_a &= \frac{I}{S} = \frac{N \cdot i_N(a) + I_{\text{fix}}}{S} \\[10pt]
				&= \frac{N \cdot i_N(a)}{S} + \frac{I_{\text{fix}}}{S} = \frac{N \cdot i_N(a)}{N \cdot a \cdot K} + \frac{I_{\text{fix}}}{S_D} \\[10pt]
				&= \frac{i_N(a)}{a \cdot K} + \frac{I_{\text{fix}}}{S_D}
			\end{align}	
			...
			\begin{mdframed}[frametitle={Erhöhung der Produktauslastung}, frametitlerule=true]
				Teilmodell:
				\begin{equation}
					\text{MIPS}_a = \frac{i_N(a)}{a \cdot K} + \frac{I_{\text{fix}}}{S_D}
				\end{equation}
				Nebenbedingung:
				\begin{equation}
					a \cdot N_{\text{max}}(a) \geq \frac{S_D}{K}.
			...
			\begin{table}
				...
				\begin{tabular}{p{1cm} p{3,5cm} p{3,5cm} p{\textwidth - 8 cm}}
					...
					$I$ & Material-Inputs & Inputs & Summe aller Materialströme, die zur Bereitstellung, Nutzung und Entsorgung des betrachteten Produkts notwendig sind \\
					$I_{\text{fix}}$ & nutzungs\textit{un}abhängige Material-Inputs & nutzungs\textit{un}abhängige Inputs & Material-Inputs, die für die Bereitstellung und die Entsorgung des betrachteten Produkts notwendig sind \\
					...
					$S$ & Service-Menge & Service & Gesamtmenge des Nutzens des betrachteten Produkts \\
					$S_D$ & Nachgefragte Service-Menge & Service-Nachfrage & Gesamtnachfrage nach Service im betrachteten Nutzungssystem \\
					$N$ & Produktnutzungs-Menge & Produktnutzung & Anzahl der Nutzungen oder Gesamtnutzungsdauer des betrachteten Produkts \\
					$N_{\text{max}}$ & Nutzungsvorrat & - & maximale Produktnutzungs-Menge des betrachteten Produkts \\
					$A$ & absolute Produktauslastung & - & erzielte Service-Menge je Produktnutzungseinheit \\ 
					$K$ & Kapazität & - & maximale absolute Produktauslastung \\
